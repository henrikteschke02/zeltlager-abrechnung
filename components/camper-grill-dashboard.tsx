...
export function CamperGrillDashboard({
  ...
  userId: string
  items?: GrillItem[]
  initialOrders?: GrillOrder[]
}) {
  const supabase = createClient()

  const [orders, setOrders] = useState<GrillOrder[]>(initialOrders)
  ...
  const executeBooking = async (item: GrillItem, qty: number) => {
    if (loadingId) return
    setLoadingId(item.id)
    setSelectedItem(null)

    // Optimistic insert
    const temp: GrillOrder = {
      id: crypto.randomUUID(),
      user_id: userId,
      grill_item_id: item.id,
      quantity: qty,
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mt-8 sticky top-24 z-40 mb-4 max-w-2xl mx-auto">
        ...
        >
            ...
            <div className="flex justify-between items-end">
              <div>
                <CardTitle className="text-[10px] font-sans font-semibold uppercase tracking-widest opacity-60 flex items-center gap-1">
                  {showDaily ? "Tages-Grill-Deckel" : "Gesamt-Grill-Deckel"}
                  <ChevronDown
          ...
          <CardContent className="relative z-10 pb-4 pt-0">
            <div className="flex items-center gap-3 rounded-2xl p-2.5 mt-2 border bg-black/5 border-black/5">
          ...
          >
            ...
            <div className="relative w-full aspect-square overflow-hidden bg-[#4c503d]">
              {loadingId === item.id ? (
                <div className="absolute inset-0 flex items-center justify-center bg-black/30 z-20">
                  <Loader2 className="w-10 h-10 animate-spin text-[#D9FF3D]" />
                </div>
              ) : (
                <Image
            ...
            <div className="flex items-center justify-between p-3 gap-2">
              <div className="min-w-0">
                ...
                <p className="text-xs font-sans font-semibold uppercase tracking-wider mt-0.5 text-[#4c503d]/50 dark:text-white/50">
      ...
      <div className="max-w-2xl mx-auto">
        <h2 className="text-xs font-sans font-semibold uppercase tracking-widest mb-3 px-1 text-muted-foreground">Meine Statistiken</h2>
          ...
          <CardContent className="p-0">
            {personalStats.length === 0 ? (
              <div className="p-8 flex flex-col items-center justify-center text-center text-muted-foreground animate-in fade-in duration-500">
                ...
                <p className="font-semibold mb-1 text-foreground/80">Noch sieht es hier leer aus!</p>
                ...
                {personalStats.map((stat, i) => (
                  <div key={i} className="flex justify-between items-center p-4">
                    <div className="flex items-center gap-3">
                      <div className="font-bold w-8 h-8 rounded-full flex items-center justify-center bg-primary/10 text-primary">
                        {stat.count}x
                      </div>
